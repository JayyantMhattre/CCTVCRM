...
internal sealed class AssignTicketCommandHandler(
    ...
    IUnitOfWork unitOfWork) : IRequestHandler<AssignTicketCommand, Result<TicketDetailDto>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var authError = await TicketAuthorization.EnsureCanAssignAsync(
        ...
        if (!await engineerLookup.ExistsAsync(request.EngineerId, cancellationToken))
            return Error.Validation("Tickets.InvalidEngineer", "Engineer not found.");

        try
        {
            ticket.AssignEngineer(TicketAssignmentId.New(), request.EngineerId, request.UserId);
            await unitOfWork.SaveChangesAsync(cancellationToken);
            return TicketMapper.ToDetail(ticket);
        }
        catch (InvalidOperationException ex)
        {
            return Error.Validation("Tickets.AssignFailed", ex.Message);
        }
    }
}
