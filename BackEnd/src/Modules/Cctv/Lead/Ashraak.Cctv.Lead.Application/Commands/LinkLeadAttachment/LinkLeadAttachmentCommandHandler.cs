...
internal sealed class LinkLeadAttachmentCommandHandler(
    ILeadRepository repository,
    IAuthPermissionChecker permissionChecker,
    IFeatureFlagService featureFlags,
    IUnitOfWork unitOfWork) : IRequestHandler<LinkLeadAttachmentCommand, Result<LeadAttachmentDto>>
{
    public async Task<Result<LeadAttachmentDto>> Handle(
        LinkLeadAttachmentCommand request,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            var attachment = lead.LinkAttachment(request.FileId, request.Title, request.UserId);
            await unitOfWork.SaveChangesAsync(cancellationToken);
            return LeadMapper.ToAttachment(attachment);
        }
