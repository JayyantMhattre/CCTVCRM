...
internal sealed class PublishPlanVersionCommandHandler(
    IAmcPlanRepository repository,
    IAuthPermissionChecker permissionChecker,
    IFeatureFlagService featureFlags,
    IUnitOfWork unitOfWork) : IRequestHandler<PublishPlanVersionCommand, Result<AmcPlanVersionDetailDto>>
{
    public async Task<Result<AmcPlanVersionDetailDto>> Handle(
        PublishPlanVersionCommand request,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            plan.PublishVersion(AmcPlanVersionId.From(request.VersionId), request.UserId);
        ...
        {
            return Error.Validation("AmcPlans.PublishFailed", ex.Message);
        }
    }
}
