...
internal sealed class CancelContractCommandHandler(
    IAmcPlanRepository planRepository,
    IAmcContractRepository contractRepository,
    IAuthPermissionChecker permissionChecker,
    IFeatureFlagService featureFlags,
    IUnitOfWork unitOfWork) : IRequestHandler<CancelContractCommand, Result<AmcContractDetailDto>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var concurrencyError = AmcConcurrencyHelper.EnsureRowVersion(request.RowVersion, contract.RowVersion);
        if (concurrencyError is not null)
            return concurrencyError;

        contract.Cancel(request.UserId);
