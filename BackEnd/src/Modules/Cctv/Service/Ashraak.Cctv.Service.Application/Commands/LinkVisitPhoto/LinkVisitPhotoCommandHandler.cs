...
internal sealed class LinkVisitPhotoCommandHandler(
    ...
    IUnitOfWork unitOfWork) : IRequestHandler<LinkVisitPhotoCommand, Result<VisitDetailDto>>
{
    public async Task<Result<VisitDetailDto>> Handle(
        LinkVisitPhotoCommand request,
        CancellationToken cancellationToken)
    {
        ...
        if (!ServiceMapper.TryParsePhotoCategory(request.Category, out var category))
            return Error.Validation("Visits.InvalidPhotoCategory", "Invalid photo category. Selfie must use the selfie endpoint.");
        ...
        try
        {
            ...
            visit.LinkPhoto(request.FileId, category, request.Caption, capturedAt, request.UserId);
        ...
        {
            return Error.Validation("Visits.LinkPhotoFailed", ex.Message);
        }
    }
}
