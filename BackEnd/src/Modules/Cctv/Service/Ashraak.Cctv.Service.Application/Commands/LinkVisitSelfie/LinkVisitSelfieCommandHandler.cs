...
internal sealed class LinkVisitSelfieCommandHandler(
    ...
    IUnitOfWork unitOfWork) : IRequestHandler<LinkVisitSelfieCommand, Result<VisitDetailDto>>
{
    public async Task<Result<VisitDetailDto>> Handle(
        LinkVisitSelfieCommand request,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            visit.LinkSelfie(request.FileId, capturedAt, request.UserId);
        ...
        {
            return Error.Validation("Visits.LinkSelfieFailed", ex.Message);
        }
    }
}
