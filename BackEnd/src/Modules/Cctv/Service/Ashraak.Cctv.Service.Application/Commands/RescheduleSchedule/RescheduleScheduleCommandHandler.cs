...
internal sealed class RescheduleScheduleCommandHandler(
    IServiceScheduleRepository scheduleRepository,
    IServiceVisitRepository visitRepository,
    IAuthPermissionChecker permissionChecker,
    IFeatureFlagService featureFlags,
    IUnitOfWork unitOfWork) : IRequestHandler<RescheduleScheduleCommand, Result<ScheduleDetailDto>>
{
    public async Task<Result<ScheduleDetailDto>> Handle(
        RescheduleScheduleCommand request,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            schedule.Reschedule(request.NewScheduledDate, request.Reason, request.UserId);
        ...
        {
            return Error.Validation("Schedules.RescheduleFailed", ex.Message);
        }
    }
}
