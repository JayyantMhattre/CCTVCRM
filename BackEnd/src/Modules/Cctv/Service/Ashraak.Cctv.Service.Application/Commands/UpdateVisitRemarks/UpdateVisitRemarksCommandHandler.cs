...
internal sealed class UpdateVisitRemarksCommandHandler(
    ...
    IUnitOfWork unitOfWork) : IRequestHandler<UpdateVisitRemarksCommand, Result<VisitDetailDto>>
{
    public async Task<Result<VisitDetailDto>> Handle(
        UpdateVisitRemarksCommand request,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            visit.UpdateRemarks(request.Remarks, request.UserId);
        ...
        {
            return Error.Validation("Visits.UpdateRemarksFailed", ex.Message);
        }
    }
}
