...
internal sealed class ReturnVisitCommandHandler(
    IServiceVisitRepository visitRepository,
    IServiceScheduleRepository scheduleRepository,
    IAuthPermissionChecker permissionChecker,
    IFeatureFlagService featureFlags,
    IUnitOfWork unitOfWork) : IRequestHandler<ReturnVisitCommand, Result<VisitDetailDto>>
{
    public async Task<Result<VisitDetailDto>> Handle(
        ReturnVisitCommand request,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            visit.ReturnForRework(request.UserId, request.ReturnReason);
        ...
        {
            return Error.Validation("Visits.ReturnFailed", ex.Message);
        }
    }
}
