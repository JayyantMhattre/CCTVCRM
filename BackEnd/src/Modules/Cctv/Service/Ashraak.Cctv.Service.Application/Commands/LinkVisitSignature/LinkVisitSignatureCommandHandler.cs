...
internal sealed class LinkVisitSignatureCommandHandler(
    ...
    IUnitOfWork unitOfWork) : IRequestHandler<LinkVisitSignatureCommand, Result<VisitDetailDto>>
{
    public async Task<Result<VisitDetailDto>> Handle(
        LinkVisitSignatureCommand request,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            visit.LinkSignature(request.FileId, request.SignerName, capturedAt, request.UserId);
        ...
        {
            return Error.Validation("Visits.LinkSignatureFailed", ex.Message);
        }
    }
}
