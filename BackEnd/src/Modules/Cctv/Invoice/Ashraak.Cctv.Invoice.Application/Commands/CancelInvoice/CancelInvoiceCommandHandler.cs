...
internal sealed class CancelInvoiceCommandHandler(
    ...
    IUnitOfWork unitOfWork) : IRequestHandler<CancelInvoiceCommand, Result<InvoiceDetailDto>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            invoice.Cancel(request.Reason, request.UserId);
            await unitOfWork.SaveChangesAsync(cancellationToken);
            return InvoiceMapper.ToDetail(invoice);
        }
        catch (InvalidOperationException ex)
        {
            return Error.Validation("Invoices.CancelFailed", ex.Message);
        }
    }
}
